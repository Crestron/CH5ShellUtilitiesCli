...
export class Ch5CreateProjectCliCustom extends Ch5BaseClassForCliNew implements ICh5CliNew {
  ...
  public constructor(public showOutputMessages: boolean = true) {
    super("create-project-components");
  }
    ...
    if (this.inputArgs["config"].argsValue !== "") {
      ...
      for (let i: number = 0; i < pagesToBeCreated.length; i++) {
        ...
        if (pageContentData) {
          ...
          const contentToChange: string = "<!-- BEGIN::CHANGEAREA - your page html markup goes here -->";
          await this.updateFileContent(path.join(pathToCreateProject, "app", "project", "components", "pages", pagesToBeCreated[i].pageName, pagesToBeCreated[i].pageName + ".html"), contentToChange, contentToChange + "\n" + newContent);
        }
      }
  ...
  private async updateFileContent(templateFileCompletePath: string, stringToReplace: string, contentToAdd: string) {
    let actualContent = fsExtra.readFileSync(templateFileCompletePath);
