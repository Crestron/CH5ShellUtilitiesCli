...
const importAll = {
    ...
    if (this.processArgs["all"] === true) {
      ...
      for (let i = 0; i < this.outputResponse.data.inputFileExistsInSourceFolder.length; i++) {
        ...
        if (path.normalize(this.outputResponse.data.inputFileExistsInSourceFolder[i]).toLowerCase().indexOf(path.normalize("./app/project/components/pages/")) >= 0) {
          ...
          fsExtra.copySync(fromPathFile, toPathFile, { recursive: true, overwrite: this.outputResponse.data.overwriteFiles, filter: file => { if (path.basename(file).toLowerCase() === "project-config.json") { return false; } else { return true; } } });
        }
      }
    }
  ...
  */
   ...
   */
  async processExportedProjectConfigJSON(selectedPages, selectedWidgets) {
    if (this.processArgs["all"] === true) {
      const jsonToBeImported = fsExtra.readJSONSync(path.resolve(this.folderPaths.temporaryLocationForProjectConfig));
      logger.log("jsonToBeImported", jsonToBeImported);
      projectConfig.addPagesToJSON(jsonToBeImported.pages);
      projectConfig.addWidgetsToJSON(jsonToBeImported.widgets);
    } else {
      const jsonToBeImported = fsExtra.readJSONSync(path.resolve(this.folderPaths.temporaryLocationForProjectConfig));
      logger.log("jsonToBeImported", jsonToBeImported);
      ...
      if (selectedPages.length > 0) {
        ...
        for (let i = 0; i < selectedPages.length; i++) {
          pageArray.push(jsonToBeImported.pages.find(tempObj => tempObj.fileName.trim().toLowerCase() === selectedPages[i].trim().toLowerCase()));
        }
        if (pageArray.length > 0) {
          projectConfig.addPagesToJSON(pageArray);
        }
      }
      ...
      if (selectedWidgets.length > 0) {
        ...
        for (let i = 0; i < selectedWidgets.length; i++) {
          pageArray.push(jsonToBeImported.widgets.find(tempObj => tempObj.fileName.trim().toLowerCase() === selectedWidgets[i].trim().toLowerCase()));
        }
        if (pageArray.length > 0) {
          projectConfig.addWidgetsToJSON(pageArray);
        }
      }
    }
    logger.log("project-config read done");
